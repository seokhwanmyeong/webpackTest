...
function get_DATA_USER_TYPE(callback) {
  getGlobalLMSData("USER_TYPE", callback);
}
...
function callUserType(response) {
  handleUserType(response);
}
...
function handleUserType(response) {
  console.log("call UserType Response:", response);

  if (response) {
    const result = JSON.parse(response);
    console.log("USER_TYPE 결과:", result);

    if (result.key === "USER_TYPE") userType = result?.value;
    else console.error("USER_TYPE 키가 아닙니다.");
  } else console.error("USER_TYPE 응답이 없습니다.");
}
