  ...
  manageAudioScript(key, group) {
    if (
      ...
      if (!this.subtitleText) {
        ...
        let centerY = this.cameras.main.height - 100;

        this.subtitleText = this.add
          .text(centerX, centerY, "", styleAudioScript)
          .setOrigin(0.5, 0.5);

        let maskShape = this.add.graphics();
        maskShape.fillRoundedRect(
          centerX - this.subtitleText.width / 2 - 40,
          centerY - this.subtitleText.height / 2 + 12,
          this.subtitleText.width + 80,
          this.subtitleText.height - 32,
          10
        );
      ...
      this.subtitleEvent = this.time.addEvent({
        ...
        callback: () => {
          ...
          if (this.subtitleText && !this.subtitleText.visible)
    ...
    if (group === "audioChoice") {
    } else if (group === "audioQuestion" || group === "audioAnswer") {
    }
  }
